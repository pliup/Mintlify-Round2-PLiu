---
title: Introduction
description: 'Welcome to Deepgram Docs! Build with our powerful text-to-speech, speech-to-text, and intelligence APIs.'
---

## Deepgram APIs


<CardGroup cols={3}>
  <Card
    title="Speech to Text"
    icon="file-lines" color="#d41cbc"
    href="/"
  >
    Transcribe Audio
      * [Pre-recorded](https://developers.deepgram.com/docs/getting-started-with-pre-recorded-audio)
      * [Streaming](https://developers.deepgram.com/docs/getting-started-with-live-streaming-audio)
      * [Models and Languages](https://developers.deepgram.com/docs/models-languages-overview)
  </Card>
  <Card
    title="Text to Speech"
    icon="file-audio" color="#d41cbc"
    href="/"
  >
    Generate Voice
    * [Getting Started](https://developers.deepgram.com/docs/text-to-speech)
    * [Voices](https://developers.deepgram.com/docs/tts-models)
  </Card>
  <Card
    title="Intelligence"
    icon="brain-circuit" color="#d41cbc"
    href="/"
  >
    Gain Insights
    * [Audio-to-text](https://developers.deepgram.com/docs/audio-intelligence)
    * [Text-to-text](https://developers.deepgram.com/docs/text-intelligence)
  </Card>
</CardGroup>

## Building Blocks

<CardGroup cols={3}>
  <Card
    title="Voicebot with analytics"
    icon="headphones" color="#d41cbc"
    href="/"
  >
    Create an end-to-end voice bot which can:
    Listen to customers: [Streaming Guide](https://developers.deepgram.com/docs/getting-started-with-live-streaming-audio)
    Understand customer: [Intent Recognition](https://developers.deepgram.com/docs/intent-recognition)
    Respond to customer: [Text-to-speech Guide](https://developers.deepgram.com/docs/text-to-speech)
    Alert on low sentiment [Sentiement Analysis](https://developers.deepgram.com/docs/sentiment-analysis)
  </Card>
  <Card
    title="Intelligent agent assist"
    icon="sparkles" color="#d41cbc"
    href="/"
  >
    Create an intelligent agent assist system:
    Listen to customer: [Streaming Guide](https://developers.deepgram.com/docs/getting-started-with-live-streaming-audio)
    Understand customer: [Intent Recognition](https://developers.deepgram.com/docs/intent-recognition)
    Identify low sentiment: [Sentiement Analysis](https://developers.deepgram.com/docs/sentiment-analysis)
    Summarize for quick access: [Summarization](https://developers.deepgram.com/docs/summarization)
  </Card>
  <Card
    title="Live Captions"
    icon="closed-captioning" color="#d41cbc"
    href="/"
  >
    Create captions:
    [Generate WebVTT & SRT Captions Guide](https://developers.deepgram.com/docs/automatically-generating-webvtt-and-srt-captions)
    [Deepgram's Python Captions OSS Package](https://github.com/deepgram/deepgram-python-captions)
    [Deepgram's Javascript Captions OSS Package](https://github.com/deepgram/deepgram-js-captions)
  </Card>
</CardGroup>

## SDKs

<CardGroup cols={4}>
  <Card
    title="Javascript"
    icon="js" color="#f7df1c"
    href="https://developers.deepgram.com/docs/js-sdk"
  >
  </Card>
  <Card
    title="Python"
    icon="python" color="#316792"
    href="https://developers.deepgram.com/docs/python-sdk"
  >
  </Card>
  <Card
    title="Go"
    icon="golang" color="#05acd7"
    href="https://developers.deepgram.com/docs/go-sdk"
  >
  </Card>
  <Card
    title=".NET"
    icon="code" color="#512bd4"
    href="https://developers.deepgram.com/docs/dotnet-sdk"
  >
  </Card>
</CardGroup>

## Resources
<CardGroup cols={3}>
  <Card
    title="Explore"
    icon="rocket" color="#d41cbc"
    href="https://developers.deepgram.com/docs/js-sdk"
  >
  * [API Playground](https://playground.deepgram.com/)
  * [API Reference](https://developers.deepgram.com/reference/deepgram-api-overview)
  </Card>
  <Card
    title="Self Host"
    icon="screwdriver-wrench" color="#d41cbc"
    href="https://developers.deepgram.com/docs/python-sdk"
  >
  * [Get started on-prem](https://developers.deepgram.com/docs/on-prem-introduction)
  * [On-prem add-ons](https://developers.deepgram.com/docs/introduction-1)
  </Card>
  <Card
    title="Community"
    icon="people-group" color="#d41cbc"
    href="https://developers.deepgram.com/docs/go-sdk"
  >
  * [Discord](https://dpgr.am/discord)
  * [Discussion Forums](https://github.com/orgs/deepgram/discussions)
  </Card>
</CardGroup>
